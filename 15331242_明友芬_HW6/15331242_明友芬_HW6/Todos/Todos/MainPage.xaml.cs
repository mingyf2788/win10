...
namespace Todos
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        public MainPage()
        {
            ...
            Services.DbContext sqldata = new Services.DbContext();
        ...
        private void WideCreateAppBarButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (createButton.Content.ToString() == "Create")
            {
                if (flag == true)
                {
                    ...
                    {
                        //关于Uri的一点知识
                        //C#中Uri的作用：提供统一资源标识符 (URI) 的对象表示形式和对 URI 各部分的轻松访问。
　　                    //简单来说就是路径信息，比如图片路径就可以通过uri来获取
                        BitmapImage newimage = Image_.Source as BitmapImage;
                        Uri imageuri = new Uri(Models.TodoItem.defaultImagePath);
                        ViewModel.AddTodoItem(title.Text, details.Text, imageuri, newimage, datepicker.Date.DateTime);
                        tile_create();
            ...
            {   if (flag == true)
                {
                    BitmapImage newimage = Image_.Source as BitmapImage;
                    Uri imageuri = new Uri(Models.TodoItem.defaultImagePath);
                    ViewModel.UpdateTodoItem(ViewModel.SelectedItem.getID(), title.Text.ToString(), 
                        details.Text.ToString(), this.ViewModel.SelectedItem.imageuri, newimage, datepicker.Date.DateTime);
                    Frame.Navigate(typeof(MainPage), ViewModel);
                    tile_create();
                    }
            }
      ...
      void OnShareDataRequested(DataTransferManager sender, DataRequestedEventArgs args) {
        ...
        private async void SearchButton_click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (item.title.Contains(Searchbox.Text))
                {
                    strb.AppendLine("Title:" + item.title + " Description:" 
                        + item.description + " Time:" + item.DATE.ToString());
                    flag = true;
                }
            }
            if (flag)
            {
                var i = await new MessageDialog(strb.ToString()).ShowAsync();
            } else
            {
